        ...
        <div class="pull-right like-box">
            <a href="javascript:;" onclick="likePost({{ $post->id }})" class="like-text">
                @if($post->checkLike($user->id))
                    <i class="fa fa-heart"></i> <span translate="no">¡No me gusta!</span>
                @else
                    <i class="fa fa-heart-o"></i> <span translate="no">¡Me gusta!</span>
                @endif
            </a>
            <div class="clearfix"></div>
            <a href="javascript:;" class="all_likes" onclick="showLikes({{ $post->id }})"><span>{{ $post->getLikeCount() }} @if($post->getLikeCount() > 1){{ 'Me gusta' }}@else{{ 'Me gusta' }}@endif</span></a>
        </div>
        <div class="clearfix"></div>
        <span>
        ...
        <div class="media post-write-comment">
            <form id="form-new-comment">
                ...
                <div class="media-body">
                    <textarea class="form-control" rows="1" placeholder="Escribe un comentario..."></textarea>
                </div>
